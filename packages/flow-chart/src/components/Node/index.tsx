...
export class Node extends React.Component<NodeProps> {
  constructor(props: NodeProps) {
    ...
    this.virtualHeight =
      this.height + sizeConfig.nodeConfig.longitudinalSpacing;
  }
  // 业务数据
  ...
  public getPoint = (isSvg?: boolean) => {
    ...
    const centerX = isSvg
      ? this.getCenterX() + rootPipelineWidth / 2
      : this.getCenterX();
    ...
    return {
      startNode: {
        virtualTopLeft: {
          ...
          y:
            centerY -
            nodeBoxHeight / 2 +
            this.nodeBox.nodeBoxConfig.longitudinalSpacing / 2,
        },
        virtualTopCenter: {
          x: centerX,
          y:
        ...
        topCenter: {
          ...
          y:
            ...
            nodeBoxHeight / 2 +
            this.nodeBox.nodeBoxConfig.longitudinalSpacing / 2 +
            this.nodeConfig.longitudinalSpacing / 2,
        },
        bottomCenter: {
          x: centerX,
          y:
            ...
            nodeBoxHeight / 2 -
            this.nodeBox.nodeBoxConfig.longitudinalSpacing / 2 +
            this.nodeBoxConfig.nodeSelfHieght -
            this.nodeConfig.longitudinalSpacing / 2,
        },
        virtualBottomCenter: {
          x: centerX,
      ...
      endNode:
        this.nodeBox.isBranchOrGroup && this.nodeBox.hasEnd
          ? {
              virtualTopLeft: {
                x: centerX - this.virtualWidth / 2,
                y:
                  centerY +
                  nodeBoxHeight / 2 -
                  this.nodeBox.nodeBoxConfig.nodeSelfHieght +
                  this.nodeBox.nodeBoxConfig.longitudinalSpacing / 2,
              },
              virtualTopCenter: {
                x: centerX,
    ...
    return this.nodeBox.isBranch
      ? [
          startPosition,
          {
            top: {
              x: x + this.width / 2,
              y: y + nodeBoxHeight - this.virtualHeight,
            },
            bottom: {
              x: x + this.width / 2,
              y: y + nodeBoxHeight - this.virtualHeight + this.height,
            },
          },
        ]
  ...
  public drawLine = () => {
    ...
    return (
      <g>
        <DrawLine
          start={startNode.virtualTopCenter}
          end={startNode.topCenter}
          endArrow
        />
        <DrawLine
          start={startNode.bottomCenter}
          end={startNode.virtualBottomCenter}
        />
        {endNode && (
          <DrawLine
            start={endNode.virtualTopCenter}
            end={endNode.topCenter}
            endArrow
          />
        )}
        {endNode && (
          <DrawLine
            start={endNode.bottomCenter}
            end={endNode.virtualBottomCenter}
          />
        )}
      </g>
    );
  ...
  public render() {
    ...
    return (
      <GlobalContext.Consumer>
        {({ deldeteNode, heightLightNodes }) => {
          const isHeightLightNode = heightLightNodes.some(
            (node) => node.id === this.nodeData.id
          );
          return (
            <>
              <div
              ...
              >
                <div
                  ...
                  style={{
                    ...
                    margin: `${this.nodeConfig.longitudinalSpacing / 2}px ${
                      this.nodeConfig.transverseSpacing / 2
                    }px`,
                  }}
                >
                  <Button
                    ...
                    type="link"
                    icon={<CloseOutlined />}
                    onClick={() => deldeteNode({ nodeBox: this.nodeBox })}
                    size={"small"}
                  />
                  {isHeightLightNode ? "12" : "2s"}
                  {this.nodeData.displayName}
                  <br />
                  {this.nodeData.type}
                  <br />
                  {this.nodeBox.path}
                  <br />
                  {this.nodeBox.parentPipeline.path}
                  <br />
                </div>
              </div>
              {this.nodeBox.typeConfig?.branch?.hasEnd ||
              this.nodeBox.typeConfig?.group?.hasEnd ? (
                <div
                  style={{
                    width: this.virtualWidth + "px",
                    height: this.virtualHeight + "px",
                    ...
                    left: startNode.virtualTopLeft.x + "px",
                    top:
                      startNode.virtualTopLeft.y +
                      this.nodeBox.getHeight() -
                      this.nodeBox.nodeBoxConfig.nodeSelfHieght +
                      "px",
                    // border: '1px solid #f00',
                  }}
